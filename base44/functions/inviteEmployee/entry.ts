...
Deno.serve(async (req) => {
  try {
    ...
    // Store the intended role so it's applied automatically when they register
    await base44.asServiceRole.entities.PendingRoleAssignment.create({ email, role: inviteRole });
