...
TARGET: 5-8 diverse, realistic recommendations that genuinely exist or existed recently`;
    ...
    if (action === 'save_recommendations') {
      const {
        ...
        filters, has_vocational_facts, search_summary, grounding_note, custom_instructions
      } = body;
      ...
      await base44.asServiceRole.entities.Activity.create({
        ...
        description: `${saved.length} grounded recommendations generated by ${user.full_name || user.email}. ${has_vocational_facts ? 'VFP grounded. ' : ''}${(assessmentsUsed || []).length} assessment(s) used.`,
        metadata: {
          batch_id: bId,
          created_at: savedAt,
