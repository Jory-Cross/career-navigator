...
import {
  DISCOVERY_ACTIVITY_SECTIONS,
  DISCOVERY_ACTIVITY_META,
} from "@/lib/assessments/discoveryActivityDefinition";
...
function sortRecords(records) {
  return [...records].sort((a, b) => {
    const aDate =
      a?.responses?.activity_date ||
      a?.updated_date ||
      a?.created_date ||
      "";

    const bDate =
      b?.responses?.activity_date ||
      b?.updated_date ||
      b?.created_date ||
      "";
...
export default function DiscoveryActivityPanel({
  clientId,
  records = [],
  onSaved,
}) {
  ...
  const activityDefinition = {
    key: DISCOVERY_ACTIVITY_META.assessment_type,
    label: DISCOVERY_ACTIVITY_META.label,
    type: "structured",
    available: true,
  ...
  const handleOpenActivity = (recordId) => {
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          ...
          >
            New Discovery Activity
          </Badge>
        </div>

        <StructuredAssessmentWorkspacePanel
          key={`new-discovery-activity-${newSession}`}
          clientId={clientId}
          assessment={activityDefinition}
          existingRecord={null}
          onSaved={handleSaved}
      ...
      <div className="flex flex-col gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Repeatable activity observations, community exploration,
            task-based discovery, job shadows, volunteer activities,
            and vocational theme testing.
          </p>
        </div>
        ...
        {sortedRecords.length === 0 ? (
          <div className="mt-3 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-center">
            <p className="text-sm font-medium text-slate-600">
              No Discovery activities recorded yet.
            </p>

            <p className="mt-1 text-xs text-slate-500">
              Select New Activity to create the first Discovery activity.
            </p>
          </div>
        ) : (
