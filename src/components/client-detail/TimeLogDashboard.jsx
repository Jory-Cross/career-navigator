...
function buildSecureDuplicateInput(entry) {
  const entryTypeId = String(
    entry?.entry_type_id || entry?.raw?.entry_type_id || ""
  ).trim();
  ...
  if (!entryTypeId) {
    throw new Error(
      "This legacy TimeEntry cannot be duplicated because it has no canonical EntryType."
    );
  }

  if (!employeeId) {
    throw new Error(
      "This TimeEntry cannot be duplicated because it has no employee owner."
    );
  }

  const duplicateDate = addOneDayDateOnly(entry?.date);

  if (!duplicateDate) {
    throw new Error(
      "This TimeEntry cannot be duplicated because it has no valid date."
    );
  }
  ...
  const entryTypeCode =
    ...
    normalizeEntryTypeCode(entry?.raw?.entry_type_code);
  ...
  return {
    ...
    location: entry?.location || entry?.raw?.location || null,
    description:
      entry?.description ||
      entry?.notes ||
